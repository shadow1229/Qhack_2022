...
def test_epoch(env, epoch, batchsize=4,dat_idx=999):
    with torch.no_grad():
        run_epoch(env, epoch, train=False, batchsize=batchsize,dat_idx = dat_idx)

def train_epoch(env, epoch, batchsize=4,dat_idx=0):
    run_epoch(env, epoch, train=True, batchsize=batchsize,dat_idx=dat_idx)
    
def run_epoch(env, epoch, train=True, batchsize=4 ,dr=None, dat_idx=0):
    ...
    with open('data_%03d.pic'%dat_idx,'wb') as f:
...
def training(nets,names ,trainset=[],testset=[],n_grid=32, batchsize=4):
    ...
    for net_idx, net in enumerate(nets):
        ...
        for epoch in range(100):  # loop over the dataset multiple times
            train_epoch(env_train, epoch,batchsize=batchsize,dat_idx=epoch)
            #test_epoch(env_test, epoch,dat_idx=999)

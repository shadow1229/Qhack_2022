...
def evaluation(env, dat = None, batchsize=4):
    with torch.no_grad():
        evaluation_epoch(env, -1, train=False, dat = dat, batchsize=batchsize)
...
def evaluation_epoch(env, epoch, train=True, dat= None, batchsize=4 ,dr=None):
    # trainset / testset = [0,{'C':[vecs] , 'N':[vecs], 'O':[vecs] } ]
    #0: HOH , 1: NA. train: 400 cases HOH, NA each, test: 54 cases each
    sets      = env['sets']
    net       = env['net']
    ...
    if dat == None:
        print('NO DATA!!')
        print('1. python grid_prebuilder.py #requires pytorch ')
        print('2. python quanv_preprocessing.py #requires pytorch + pennylane')
        raise ValueError
        #for i in range(len(sets) //batchsize):
        #    print(i)
        #    input_batch.append([build_cuda(sets[batchsize*i+j][1],n_grid=n_grid) for j in range(batchsize)])
        ...
        #use pre-built dat (quantum version uses this)
        for i in range(len(dat) //batchsize):
            input_batch.append([ dat[batchsize*i + j ][1] for j in range(batchsize)])
            answer_batch.append( np.array([ [[[[[ dat[batchsize*i + j][0] ]]]]] for j in range(batchsize)]))
    ...
    for i in range(len(answer_batch)):
        ...
        #get result
        np_answers_reshape = np.array( [np_answers[i][0][0][0][0] for i in range(np_answers.shape[0] )] )
        outputs = net.eval()(inputs) 
        np_outputs = outputs.cpu().detach().numpy()
        outputs_sigmoid = np.array( [ sigmoid(np_outputs[i][0][0][0][0]) for i in range(np_outputs.shape[0])])
        for i in range(batchsize):
            if np_answers_reshape[i] == 1 and outputs_sigmoid[i] <0.5:
                n_10 += 1
            elif np_answers_reshape[i] == 1 and outputs_sigmoid[i] >= 0.5:
                n_11 += 1
            elif np_answers_reshape[i] == 0 and outputs_sigmoid[i] <0.5:
                n_00 += 1
            elif np_answers_reshape[i] == 0 and outputs_sigmoid[i] >= 0.5:
                n_01 += 1
    print('prediction on H2O - # correct: %3d # wrong: %3d'%(n_00, n_01)) 
    print('prediction on NA+ - # correct: %3d # wrong: %3d'%(n_11, n_10)) 
def sigmoid(x):
    return 1.0/ (1.0 + np.exp(-x))
def training(nets,names ,trainset=[],testset=[],n_grid=32, batchsize=4, suffix='4q'):
    # trainset / testset = [0,{'C':[vecs] , 'N':[vecs], 'O':[vecs] } ]
    #0: HOH , 1: NA. train: 400 cases HOH, NA each, test: 54 cases each
    for net_idx, net in enumerate(nets):
        ...
        with open('dat/data_999_%s.pic'%(suffix),'rb') as f:
            dat_test = pickle.load(f)

        for epoch in range(start_epoch, 1000):  # loop over the dataset multiple times

            with open('dat/data_%03d_%s.pic'%(epoch%100, suffix),'rb') as f:
            ...
            if epoch%50 == 49:
                #save state:
                torch.save(net.module.state_dict(), '%s/cnn_state_%05d.bin'%(name,(epoch+1)))
    
        log.close()
        del(log)
        
        evaluation(env_test, dat=dat_test, batchsize=4)
...
if __name__ == '__main__':
    ...
    nets  = [networks.Net_4q()]
    names = ['qsea_4q']
    
    training(nets,names, trainset=trainset, testset=testset, n_grid=32, batchsize=4, suffix= '4q')

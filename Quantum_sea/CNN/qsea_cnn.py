...
def test_epoch(env, epoch, batchsize=4):
    with torch.no_grad():
        run_epoch(env, epoch, train=False, batchsize=batchsize)

def train_epoch(env, epoch, batchsize=4):
    run_epoch(env, epoch, train=True, batchsize=batchsize)
    
def run_epoch(env, epoch, train=True, batchsize=4 ,dr=None):

...
def circuit(indices):
    ...
    qml.QuantumPhaseEstimation(U, target_wires, estimation_wires)
...
def number_of_solutions(indices):
    ...
    theta = max_id*np.pi/8.0
    M = 16.0*np.sin(theta/2)*np.sin(theta/2)
    return M
    # QHACK #
...
def relative_error(indices):
    ...
    # QHACK #
    pred = number_of_solutions(indices)
    rel_err = (pred - len(indices))/(len(indices)) *100.0

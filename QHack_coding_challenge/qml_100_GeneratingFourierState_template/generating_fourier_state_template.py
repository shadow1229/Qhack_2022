...
def generating_fourier_state(n_qubits, m):
    ...
    def circuit(angles):
        ...
        qml.broadcast(unitary=qml.Hadamard, pattern="single", wires=range(n_qubits))
        qml.broadcast(unitary=qml.RZ, pattern="single", wires=range(n_qubits), parameters=angles)
        # QHACK #
